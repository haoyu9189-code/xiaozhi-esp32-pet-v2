...
#define MAX_ACCEPTABLE_SEQUENCE_GAP 5      // 允许的小间隙（UDP 丢包/乱序是正常的）
#define MAX_SEQUENCE_GAP_FOR_RESET 100     // 超过此值认为是会话重启或严重网络问题

// Valid audio sample rates
static const uint32_t VALID_SAMPLE_RATES[] = {8000, 12000, 16000, 24000, 48000};

static bool IsValidSampleRate(uint32_t rate) {
    for (uint32_t valid_rate : VALID_SAMPLE_RATES) {
        if (rate == valid_rate) return true;
    }
    return false;
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        if (sequence <= remote_sequence_) {
            ESP_LOGD(TAG, "Received duplicate/old audio packet: seq=%lu, current=%lu (ignored)",
                     sequence, remote_sequence_);
            return;
        }

        // 计算序列号间隙
        uint32_t gap = sequence - remote_sequence_;

        // 处理序列号间隙
        if (gap > 1) {
            if (gap <= MAX_ACCEPTABLE_SEQUENCE_GAP) {
                // 小间隙：可能是 UDP 丢包或乱序 - 接受包
                ESP_LOGD(TAG, "Small sequence gap detected: expected=%lu, received=%lu, gap=%lu (accepted)",
                         remote_sequence_ + 1, sequence, gap - 1);
            } else if (gap <= MAX_SEQUENCE_GAP_FOR_RESET) {
                // 中等间隙：明显的丢包 - 警告但继续
                ESP_LOGW(TAG, "Significant sequence gap: expected=%lu, received=%lu, gap=%lu (may cause audio glitch)",
                         remote_sequence_ + 1, sequence, gap - 1);
            } else {
                // 大间隙：可能是会话重启或严重网络问题 - 重置序列号跟踪
                ESP_LOGW(TAG, "Very large sequence gap: expected=%lu, received=%lu, gap=%lu (sequence tracking reset)",
                         remote_sequence_ + 1, sequence, gap - 1);
            }
        }
...
void MqttProtocol::ParseServerHello(const cJSON* root) {
    ...
    if (cJSON_IsObject(audio_params)) {
        ...
        if (cJSON_IsNumber(sample_rate)) {
            int new_sample_rate = sample_rate->valueint;
            if (!IsValidSampleRate(new_sample_rate)) {
                ESP_LOGW(TAG, "Server sent invalid sample_rate %d, keeping %d", new_sample_rate, server_sample_rate_);
            } else {
                server_sample_rate_ = new_sample_rate;
                ESP_LOGI(TAG, "Server sample_rate set to %d", server_sample_rate_);
            }
        }
        auto frame_duration = cJSON_GetObjectItem(audio_params, "frame_duration");
        if (cJSON_IsNumber(frame_duration)) {
            server_frame_duration_ = frame_duration->valueint;
            ESP_LOGI(TAG, "Server frame_duration set to %d ms", server_frame_duration_);
        }
    }
    ...
    auto key_obj = cJSON_GetObjectItem(udp, "key");
    auto nonce_obj = cJSON_GetObjectItem(udp, "nonce");

    if (!server || !cJSON_IsString(server) || !port || !cJSON_IsNumber(port) ||
        !key_obj || !cJSON_IsString(key_obj) || !nonce_obj || !cJSON_IsString(nonce_obj)) {
        ESP_LOGE(TAG, "Invalid UDP configuration, missing required fields");
        return;
    }

    udp_server_ = server->valuestring;
    udp_port_ = port->valueint;
    auto key = key_obj->valuestring;
    auto nonce = nonce_obj->valuestring;
