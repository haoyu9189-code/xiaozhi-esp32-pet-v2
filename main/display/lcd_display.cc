...
// Common helper methods used by both WeChat and non-WeChat styles
void LcdDisplay::CreateTopBar(lv_obj_t* parent, const lv_font_t* icon_font) {
...
void LcdDisplay::CreateStatusBar(lv_obj_t* screen) {
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    ...
    lv_obj_align(status_bar_, LV_ALIGN_TOP_MID, 0, 1);
...
void LcdDisplay::CreateBottomBar(lv_obj_t* screen, const lv_font_t* text_font, const lv_font_t* large_icon_font) {
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);

    bottom_bar_ = lv_obj_create(screen);
    lv_obj_set_width(bottom_bar_, LV_HOR_RES);
    lv_obj_set_height(bottom_bar_, LV_SIZE_CONTENT);
    lv_obj_set_style_min_height(bottom_bar_, 25, 0);
    lv_obj_set_style_radius(bottom_bar_, 0, 0);
    lv_obj_set_style_bg_opa(bottom_bar_, LV_OPA_TRANSP, 0);
    lv_obj_set_style_text_color(bottom_bar_, lvgl_theme->text_color(), 0);
    lv_obj_set_style_pad_top(bottom_bar_, lvgl_theme->spacing(1), 0);
    lv_obj_set_style_pad_bottom(bottom_bar_, lvgl_theme->spacing(1), 0);
    ...
    lv_obj_align(chat_message_label_, LV_ALIGN_CENTER, 0, -2);
    // Limit to single line height for horizontal-only scrolling
    lv_obj_set_height(chat_message_label_, lv_font_get_line_height(text_font));
}

void LcdDisplay::CreateLowBatteryPopup(lv_obj_t* screen, const lv_font_t* text_font) {
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
...
void LcdDisplay::SetupUI() {
    ...
    CreateTopBar(container_, icon_font);
    CreateStatusBar(screen);

    // Create content area for chat messages
    ...
    // Create low battery popup and emoji elements
    CreateLowBatteryPopup(screen, text_font);
...
void LcdDisplay::RotateOldMessages() {
...
void LcdDisplay::CollapseSystemMessages(const char* role) {
...
lv_obj_t* LcdDisplay::CreateMessageBubble(const char* content, const lv_font_t* text_font) {
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    ...
    // Limit to 2 lines with circular scroll
    lv_coord_t line_height = lv_font_get_line_height(text_font);
    lv_coord_t max_text_height = line_height * 2;
    lv_label_set_long_mode(msg_text, LV_LABEL_LONG_SCROLL_CIRCULAR);
    lv_obj_set_height(msg_text, max_text_height);

    lv_obj_set_width(msg_bubble, bubble_width);
    lv_coord_t padding = lvgl_theme->spacing(4) * 2;
    lv_obj_set_height(msg_bubble, max_text_height + padding);

    return msg_bubble;
}

void LcdDisplay::StyleMessageBubble(lv_obj_t* bubble, lv_obj_t* text, const char* role) {
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    ...
    if (strcmp(role, "user") == 0) {
        ...
        lv_obj_set_user_data(bubble, (void*)"user");
    } else if (strcmp(role, "assistant") == 0) {
        lv_obj_set_style_bg_color(bubble, lvgl_theme->assistant_bubble_color(), 0);
        lv_obj_set_style_bg_opa(bubble, LV_OPA_70, 0);
        lv_obj_set_style_text_color(text, lvgl_theme->text_color(), 0);
        lv_obj_set_user_data(bubble, (void*)"assistant");
    ...
    lv_obj_set_style_flex_grow(bubble, 0, 0);
}

void LcdDisplay::AlignMessageBubble(lv_obj_t* bubble, const char* role) {
    if (strcmp(role, "user") == 0 || strcmp(role, "system") == 0) {
        // Create full-width container for alignment
        ...
        lv_obj_set_parent(bubble, container);

        if (strcmp(role, "user") == 0) {
            lv_obj_align(bubble, LV_ALIGN_RIGHT_MID, -25, 0);
        } else {
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    RotateOldMessages();
    CollapseSystemMessages(role);

    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    auto text_font = lvgl_theme->text_font()->font();

    lv_obj_t* msg_bubble = CreateMessageBubble(content, text_font);
    lv_obj_t* msg_text = lv_obj_get_child(msg_bubble, 0);

    StyleMessageBubble(msg_bubble, msg_text, role);
    AlignMessageBubble(msg_bubble, role);
...
void LcdDisplay::SetupUI() {
    ...
    // Create main container as background
    container_ = lv_obj_create(screen);
    lv_obj_set_size(container_, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_radius(container_, 0, 0);
    lv_obj_set_style_pad_all(container_, 0, 0);
    ...
    // Create emoji display box (centered)
    ...
    // Create preview image (centered, hidden by default)
    preview_image_ = lv_image_create(screen);
    lv_obj_set_size(preview_image_, width_ / 2, height_ / 2);
    lv_obj_align(preview_image_, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_flag(preview_image_, LV_OBJ_FLAG_HIDDEN);

    // Create UI components using helper methods
    CreateTopBar(screen, icon_font);
    lv_obj_align(top_bar_, LV_ALIGN_TOP_MID, 0, 1);

    CreateStatusBar(screen);
    CreateBottomBar(screen, text_font, large_icon_font);
    CreateLowBatteryPopup(screen, text_font);
}
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Mutual exclusion logic: Show subtitle when there's content, otherwise show pet status
    // IMPORTANT: Delete pet status to free RAM (~3KB) when showing subtitle
    bool has_content = (content != nullptr && strlen(content) > 0);

    if (has_content) {
        // Has subtitle - show it, delete pet status to save RAM (~3KB for opus decoder)
        ESP_LOGI(TAG, "SetChatMessage: Showing subtitle, deleting pet status (role=%s, content_len=%d)",
                 role ? role : "null", (int)strlen(content));
        lv_obj_remove_flag(chat_message_label_, LV_OBJ_FLAG_HIDDEN);

        if (pet_status_container_ != nullptr) {
            ESP_LOGI(TAG, "SetChatMessage: Deleting pet_status_container to free ~3KB RAM");
            lv_obj_del(pet_status_container_);
            pet_status_container_ = nullptr;
        }
    } else {
        // No subtitle - hide it, recreate pet status if needed
        ESP_LOGI(TAG, "SetChatMessage: Hiding subtitle, showing pet status (role=%s)",
                 role ? role : "null");
        lv_obj_add_flag(chat_message_label_, LV_OBJ_FLAG_HIDDEN);

        if (pet_status_container_ == nullptr && bottom_bar_ != nullptr) {
            // Recreate pet status display
            ESP_LOGI(TAG, "SetChatMessage: Recreating pet_status_container");
            pet_status_container_ = PetStatusDisplay::Create(bottom_bar_);
            if (pet_status_container_ != nullptr) {
                lv_obj_remove_flag(pet_status_container_, LV_OBJ_FLAG_HIDDEN);

                // Immediately update with current pet stats to avoid showing 0s
                ...
                ESP_LOGD(TAG, "SetChatMessage: Updated pet status on recreation: H=%d C=%d HP=%d Coins=%d",
                         stats.hunger, stats.cleanliness, stats.happiness, coins);
            }
        } else if (pet_status_container_ != nullptr) {
            lv_obj_remove_flag(pet_status_container_, LV_OBJ_FLAG_HIDDEN);
        }
    }
}
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Update bottom bar background - transparent so no black frame around subtitle
    if (bottom_bar_ != nullptr) {
        lv_obj_set_style_bg_opa(bottom_bar_, LV_OPA_TRANSP, 0);
    }
#endif
...
void LcdDisplay::SetPetStatus(const PetStats& stats, uint8_t coins) {
    // Call base class implementation for logging
    Display::SetPetStatus(stats, coins);
    // LCD display doesn't show pet status icons by default
    // This can be overridden in board-specific CustomLcdDisplay if needed
}
