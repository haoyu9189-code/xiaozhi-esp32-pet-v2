...
// UI Layout Constants
constexpr int UI_TOAST_WIDTH = 200;
constexpr int UI_TOAST_HEIGHT = 40;
constexpr int UI_TOAST_Y_OFFSET = 20;
constexpr int UI_CLOCK_WIDTH = 200;
constexpr int UI_CLOCK_HEIGHT = 50;
constexpr int UI_CLOCK_Y_OFFSET = 15;
constexpr int UI_LISTEN_Y_OFFSET = 5;
constexpr int UI_STATUS_ICON_X_OFFSET = -120;
constexpr int UI_STATUS_ICON_Y_OFFSET = 18;
constexpr int UI_EYE_X_OFFSET = 10;
constexpr int UI_EYE_Y_OFFSET = 30;
constexpr int UI_SCROLL_SPEED = 20;
...
static void SetupUI(const gfx_handle_t engine_handle, EmoteDisplay* const display)
{
    ...
    gfx_obj_align(g_obj_anim_eye, GFX_ALIGN_LEFT_MID, UI_EYE_X_OFFSET, UI_EYE_Y_OFFSET);
    ...
    gfx_obj_align(g_obj_label_toast, GFX_ALIGN_TOP_MID, 0, UI_TOAST_Y_OFFSET);
    gfx_obj_set_size(g_obj_label_toast, UI_TOAST_WIDTH, UI_TOAST_HEIGHT);
    ...
    gfx_label_set_scroll_speed(g_obj_label_toast, UI_SCROLL_SPEED);
    ...
    gfx_obj_align(g_obj_label_clock, GFX_ALIGN_TOP_MID, 0, UI_CLOCK_Y_OFFSET);
    gfx_obj_set_size(g_obj_label_clock, UI_CLOCK_WIDTH, UI_CLOCK_HEIGHT);
    ...
    gfx_obj_align(g_obj_anim_listen, GFX_ALIGN_TOP_MID, 0, UI_LISTEN_Y_OFFSET);
    gfx_anim_start(g_obj_anim_listen);
    gfx_obj_set_visible(g_obj_anim_listen, false);

    g_obj_img_status = gfx_img_create(engine_handle);
    gfx_obj_align(g_obj_img_status, GFX_ALIGN_TOP_MID, UI_STATUS_ICON_X_OFFSET, UI_STATUS_ICON_Y_OFFSET);
...
{
    // EmoteDisplay does not support preview images
    // This is intentional - emote display only shows simple emoji animations
    if (image) {
        ESP_LOGD(TAG, "SetPreviewImage: Preview image not supported in EmoteDisplay");
    }
}
