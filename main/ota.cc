...
void Ota::MarkCurrentVersionValid() {
    ...
    if (err != ESP_OK) {
        // ESP_ERR_NOT_SUPPORTED (0x105) 表示 otadata 分区未初始化（首次烧录或factory启动）
        // ESP_ERR_NOT_FOUND 表示分区不存在
        if (err == ESP_ERR_NOT_SUPPORTED || err == ESP_ERR_NOT_FOUND) {
            ESP_LOGW(TAG, "OTA data partition not initialized (err=0x%x), this is normal for first boot", err);
        } else {
            ESP_LOGE(TAG, "Failed to get state of partition (err=0x%x)", err);
        }
        return;
    }
